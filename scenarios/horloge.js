...
networkTools.runNetwork(networkTools.parseTopology("a-b b-c c-d d-e e-f f-g g-h h-i i-j j-k k-l l-m m-n n-o o-p p-q q-r r-s s-t t-u u-v v-w w-x x-y y-z")).then(() => {
  ...
  return Promise.all([
    ...
    cliTools.execCommande(`./cli.js --port=${networkTools.network['a'].port} --bot=true set Ville Paris 1234567890`)
  ,
    // Initialisation de z avec Tokio
    cliTools.execCommande(`./cli.js --port=${networkTools.network['z'].port} --bot=true set Ville Tokio 1234567890`)
  ]);
}).then(
